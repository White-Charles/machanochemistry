...
from atoms_model import read_one_car, set_cons2, cal_LBFGC, array2dict, dict2list, set_cons1, opt_strain, vac_ext, write_atoms, fix_layers
...
def get_dic(adslab, atom_model):
    adslab = adslab.copy()
    adslab = fix_layers(adslab,fixed_atoms_layers={(-1,-1,-1):[-1]})
...
def get_dic2(adslab, atom_model):
    adslab = adslab.copy()
    adslab = fix_layers(adslab,fixed_atoms_layers={(-1,-1,-1):[-1]})
    tags = adslab.get_tags()
    position = adslab.positions
    ...
    # 获取相邻元素个数大于 2 的元素
    tri2 = [n for n in neighbor_list if len(n) < 4]
    top_dict = {f"T-{str(i+1)}": points[i] for i in range(len(points))}
    ...
    color_list =  list(tab10.keys())

    for i in range(len(relation_list)):  # i < j 之前定义好了
        for j in relation_list[i]:
            bri_dict[f"B-{str(i+1)},{str(j+1)}"] = np.mean(points[[i, j], :], axis=0)
            # bri_dict[f"bri_{str(i)}{str(j)}"] = np.mean(points[[i, j], :], axis=0)
            xs, ys = zip(*points[[i, j], :])
            # 使用 plot 函数将点连成一条线
            plt.plot(xs, ys, color=color_list[7], linewidth=2, alpha=0.5)
    hol_dict = {}  # hollow 位置的坐标
    for item in tri2:
        tri1 = item
        tri = points[tri1]
        hol_dict[f"H-{str(tri1[0]+1)},{str(tri1[1]+1)},{str(tri1[2]+1)}"] = np.mean(
        # hol_dict[f"hol_{str(tri1[0]+1)}{str(tri1[1]+1)}{str(tri1[2]+1)}"] = np.mean(
            tri, axis=0
        )
    # plt.plot(points[:, 0], points[:, 1], "o")
    ...
    for i in range(3):
        data = all_dict[i]
        for key, value in data.items():
            plt.scatter(value[0], value[1], s=150, c=color_list[i], alpha=1.0)
            plt.text(value[0]+0.1, value[1]-0.2, key, fontsize=18, fontproperties='Arial', c=color_list[i])
    # 显示图形
    plt.axis("off")
    plt.axis("equal")
    # 设置图像的 dpi（例如设置为300）
    plt.savefig('plot_points.png', dpi=500)
    plt.show()
    # 依据单原子的吸附高度补充字典中z方向的值
    mel_dict = {}
    for i in range(2):
...
# mel_d = get_dic2(adslab, one_atoms_l)  # 每个点的坐标
...
def get_atom_adsmodel(adslab,mask,order=None,is_cal=False):
...
def all_same_type(lst):
    if not lst:  # 如果列表为空，返回True，因为没有不同类型的元素
        return True

    # 取列表的第一个元素的类型
    first_type = type(lst[0])

    # 检查列表中所有元素是否都与第一个元素类型相同
    return all(isinstance(item, first_type) for item in lst)

def transform_nested_list(nested_list, fun=deepcopy):
    transformed_list = []
    for sublist in nested_list:
        # 检查 sublist 是否是一个列表
        if isinstance(sublist, list) and all_same_type(sublist):
            # 对每个子列表递归地应用 transform_nested_list
            transformed_sublist = transform_nested_list(sublist, fun=fun)
        else:
            # 应用 apply_function 到单个元素上
            transformed_sublist = fun(sublist)
        transformed_list.append(transformed_sublist)
    return transformed_list


def slab_to_slabs(slab,repeat_multipy=(4,4,1)):
    # 单个slab扩展
    value_copy = slab.copy()
    value_copy.set_constraint([])
    value_copy = value_copy.repeat(repeat_multipy)
    return(value_copy)


def slab2slabs(slabs, repeat_multipy=(4, 4, 1)):
    # 传递 slab_to_slabs 函数，但不调用它
    fun =  lambda x: slab_to_slabs(x, repeat_multipy=repeat_multipy)
    slabs2 = transform_nested_list(slabs, fun=fun)
    slabs_dict = array2dict(slabs2)
    return(slabs2, slabs_dict)
...
def get_strain_adslab2(mol_rotate_l, strain_slabs_l, strain):
    # 第二种方法，采用映射的方法
    # 对 mol_rotate_l 列表中的每一个吸附构型，用 strain_slabs_l 的列表映射一下
    strain_adslab_l = []
    for i in range(len(strain_slabs_l)):
        strain_adslab_i = []
        mol_strain_i,_ = get_bulk_set(mol_rotate_l[i], strain[i])
        for j in range(len(strain_slabs_l[i])):
            t1 = strain_slabs_l[i][j].copy()
            t2 = mol_strain_i[j].copy()
            t1_Cu = (max (t1.get_positions()[t1.get_atomic_numbers()!=8,2]))
            t2_Cu = (max (t2.get_positions()[t2.get_atomic_numbers()!=8,2]))

            t3 = t1+t2[-2:] # 补充上O2
            pos = t3.get_positions()
            pos[-2:, 2] += (t1_Cu-t2_Cu)
            t3.set_positions(pos)
            # t3 = set_cons_surface_2(t3)
            strain_adslab_i.append(t3)

        strain_adslab_l.append(strain_adslab_i)
    strain_adslab_d = array2dict(strain_adslab_l)
    return (strain_adslab_l, strain_adslab_d)
