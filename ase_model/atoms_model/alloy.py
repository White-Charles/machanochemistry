...
from atoms_model import point_position, sort_z, fix_layers, array2dict
...
def get_strain_alloy_slabs(alloy_slab_l, strain_slabs_l, cycle=1):
    # alloy_slab_l 是没有应变的合金基底
    # strain_slabs_l 是有应变的纯金属基底
    # cycle 决定了是要把 strain_slabs_l 的第几层替换成 alloy_slab_l 中的合金
    if not isinstance(alloy_slab_l, list):
        alloy_slab_l = [alloy_slab_l]
    # 如果alloy_slab_l不是列表，则将其打包成列表

    strain_alloy_slabs_l = copy.deepcopy(strain_slabs_l)
    # 用于创建合金基底

    for i in range(len(strain_alloy_slabs_l)):
        for j in range(len(strain_alloy_slabs_l[i])):
            k = j if cycle == 1 else i
            an = alloy_slab_l[k % len(alloy_slab_l)].get_atomic_numbers()
            # 使用取模运算符来确保索引值不超过alloy_slab_l的长度
