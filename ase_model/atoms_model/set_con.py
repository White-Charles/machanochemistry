...
from .strain import sort_z
from ase.constraints import FixAtoms, FixCartesian

def set_cons_surface(model):
    """
    此约束的效果是：固定表面两层以下的原子
    """
    slab = model.copy()
    ...
    filtered_layers = [i for i in range(len(layers)) if layers[i] > max(layers) - 2]  #
    tags[filtered_layers] = 1  # 被吸附基底的次表面和表面
...
def fix_layers(model,fixed_atoms_layers={}):
    ...
    {(1,1,0):[0,1,2]} means the 0,1,2 layers (count from bottom) need to fix x and y cartesian
...
def set_cons_surface_fix(model, unfix=1 ):
    """
    此约束的效果是：固定原子，默认只放松一层
    """
    slab = model.copy()
    slab.set_constraint() # 清空约束
    slab1 = slab.copy()
    atomic_numbers = slab1.get_atomic_numbers()
    filtered_layers1 = [
        i for i in range(len(atomic_numbers)) if atomic_numbers[i] == 8
    ]  #
    del slab1[filtered_layers1]  # 先暂时删除氧原子
    layers = sort_z(slab1.get_positions()[:, 2])
    tags = np.zeros(len(slab1))
    filtered_layers = [i for i in range(len(layers)) if layers[i] > max(layers) - unfix]  #
    tags[filtered_layers] = 1  # 被吸附基底的次表面和表面
    slab1.set_tags(tags)  # 将tags属性替换
    # Fixed atoms are prevented from moving during a structure relaxation. We fix all slab atoms beneath the surface
    cons = FixAtoms(indices=[atom.index for atom in slab1 if (atom.tag < 1)])
    slab.set_constraint(cons)
    slab.set_pbc(True)
    return slab

def fix_layers_atoms(model, fix_layers={}, fix_atoms={}, refer_slab=None):
    """
    # description
    The effect of this constraint is to fix the cartesian of atoms of the specified layer
    ...
    fix_layers: dictionary,
    the key means which cartesian would be fixed. Negative meansreverse selection of subsequent values (relaxe).
    the value means which layers would be fixed. Negative means count from bottom.
    {(1,1,0):[0,1,2]} means the 0,1,2 layers (count from bottom) need to fix x and y cartesian
    {(-1,-1,-1):[-1,-2]} means the all layers except 0,1, layers (count from top) need to fix all cartesian
    {(1,1,1):[0,1,2]} means the 0,1,2 layers(count from bottom) need to fix all cartesian

    fix_atoms: dictionary,

    fix_layers_atoms(test2, fix_layers={(1,1,1):list(range(5))}, fix_atoms={(1,1,0):[-1]})
    The atoms of bottom five layers are fixed and the x&y cartesian of final atoms are fixed.
    """
    slab = model.copy()
    slab.set_constraint() # 清除约束
    if refer_slab is None:
        refer_slab = slab.copy()
    zlayers = sort_z(refer_slab.get_positions()[:,-1]) # z坐标分层
    tags = np.zeros(len(slab))
    alayers =  np.arange( len( set( zlayers) ) ) # 层数 唯一值
    anums = np.arange(model.get_global_number_of_atoms())
    con = [] # 存储约束
    filter = np.empty((len(fix_layers) + len(fix_atoms) +1), object)
    # 存储用于固定自由度的标记，当约束为固定某些自由度时会用到

    count_y = 0 # 防止第一个循环为空
    for count_y, (key, value) in enumerate(fix_layers.items(), start=1):
        # key 表示固定方式，正负值表示正选还是反选，value表示选择层，正负表示从下还是从上数
        value0 = alayers[value] # 允许正选和反选
        if sum(key)<0: # 反选
            filtered_layer = [i for i in range(len(zlayers)) if zlayers[i] not in value0]
            tags[filtered_layer] = count_y
        elif sum(key)>0: # 正选
            filtered_layer = [i for i in range(len(zlayers)) if zlayers[i] in value0]
            tags[filtered_layer] = count_y
            filter[count_y] = filtered_layer
    for count_a, (key, value) in enumerate(fix_atoms.items(), start=count_y+1):
        # key 表示固定方式，正负值表示正选还是反选，value表示选择层，正负表示从下还是从上数
        value0 = anums[value] # 允许正选和反选
        if sum(key)<0: # 反选
            filtered_atoms = [i for i in anums if anums[i] not in value0]
            tags[filtered_atoms] = count_a
        elif sum(key)>0: # 正选
            filtered_atoms = [i for i in anums if anums[i] in value0]
            tags[filtered_atoms] = count_a
            filter[count_a] = filtered_atoms
    slab.set_tags(tags)  # 将tags属性替换

    count_y = 0 # 防止第一个循环为空
    for count_y, (key, value) in enumerate(fix_layers.items(), start=1):
        if sum(key) == 3 or sum(key) == -3: # 反选
            cons0 = FixAtoms(indices=[atom.index for atom in slab if (atom.tag == count_y)])
            con.append(cons0)
        else:
            key0 = tuple(abs(x) for x in key)
            cons0 = FixCartesian(filter[count_y], mask=key0)
            con.append(cons0)
    for count_a, (key, value) in enumerate(fix_atoms.items(), start=count_y+1):
        if sum(key) == 3 or sum(key) == -3: # 反选
            cons0 = FixAtoms(indices=[atom.index for atom in slab if (atom.tag == count_a)])
            con.append(cons0)
        else:
            key0 = tuple(abs(x) for x in key)
            cons0 = FixCartesian(filter[count_a], mask=key0)
            con.append(cons0)
...
#     slab1.set_tags(tags)  # 将tags属性替换
#     # Fixed atoms are prevented from moving during a structure relaxation. We fix all slab atoms beneath the surface
#     cons0 = FixAtoms(indices=[atom.index for atom in slab1 if (atom.tag < 1)])
#     cons1 = FixCartesian([-2], mask=(1, 1, 0))
#     slab.set_constraint([cons0, cons1])
#     slab.set_pbc(True)
#     return slab

# def set_cons_surface_1(model):
#     """
#     此约束的效果是：固定表面层以下的原子
#     """
#     slab = model.copy()
...
#     layers = sort_z(slab1.get_positions()[:, 2])
#     tags = np.zeros(len(slab1))
#     filtered_layers = [i for i in range(len(layers)) if layers[i] > max(layers) - 1]  #
#     tags[filtered_layers] = 1  # 被吸附基底的次表面和表面
#     slab1.set_tags(tags)  # 将tags属性替换
#     # Fixed atoms are prevented from moving during a structure relaxation. We fix all slab atoms beneath the surface
#     cons = FixAtoms(indices=[atom.index for atom in slab1 if (atom.tag < 1)])
