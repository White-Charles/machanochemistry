...
def get_nested_depth(obj):
    if isinstance(obj, dict):
        return 1 + max((get_nested_depth(v) for v in obj.values()), default=0)
    elif isinstance(obj, list):
        return 1 + max((get_nested_depth(item) for item in obj), default=0)
    else:
        return 0
...
def out_car_list(input, path="New_Folder", isp=False, out_format="vasp"):
    ...
    if get_nested_depth(all_model)<3:
        for key in all_model.keys():
            filepath = os.path.join(".", path, key)
            os.makedirs(filepath)
            if out_format == "vasp":
                out_poscar(all_model[key], path=filepath, isp=isp)
            elif callable(out_format):
                out_format(filepath, all_model[key] )

    else:
        for key in all_model.keys():
            filepath = os.path.join(".", path, key)
            os.makedirs(filepath)
            out_car_list(all_model[key], path=filepath, isp=isp, out_format=out_format)
    ...
    if callable(car): # 如果car是一个函数
        ...
        if os.path.isfile(filename):
            if "OSZICAR" in car:
                ...
                _, atom = get_dos_atom_orbitals(path, 0)
            elif "OUTCAR" in car:
...
def read_cars(source_folder, car="CONTCAR",isp=False):
    if isinstance(car, str):
        ...
        car_path = 'CAR'
    dict_file = os.path.join(source_folder, car_path + 'S.pkl')
    if os.path.exists(dict_file):
        print(dict_file)
        with open(dict_file, "rb") as file:
            atoms_d = pickle.load(file)
            print(atoms_d.keys())
        atoms_l = dict2list(atoms_d)
    else:
        atoms_l = []
        atoms_d = {}
        atoms_d0 = {}  # 中间变量
        ...
        with open(dict_file, "wb") as file:
            pickle.dump(atoms_d, file)
    return (atoms_l, atoms_d)

def get_dos_data(
    dos_folder, atom_num=[6], orbital_list=list(range(9)), erange=None, no_plot=True
):
    # print(dos_folder)
    # 该函数可以获得指定文件夹的dos信息
...
def get_dos_atom_orbitals(
    ...
    if erange is None:
        ...
        indices = np.where((c > erange[0]) & (c <= erange[-1]))[0]
        startpoint = indices[0]
        endpoint = startpoint+int(round(len(indices)/10)*10)

        e = c[startpoint:endpoint]
        d = dos_data[startpoint:endpoint]
